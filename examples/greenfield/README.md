...
## NOTE, the IAM part now done yet.
...
kind: Deployment
metadata:
  name: hs-s3-provisioner-deployment
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: hs-s3-provisioner
        image: zhengkaics/hs-s3-privisioner:v1.0.3
...
**NOTE:**  `generateBucketName` and `bucketName` should not appear together
```
 # kubectl create -f obc.yaml
objectbucketclaim.objectbucket.io/myobc created
```
## NOTE: following does not work since IAM part not finished yet.
